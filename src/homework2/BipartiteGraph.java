...
// reference from internet: https://github.com/Satshabad/Simple-Graph-Implementation

import java.util.ArrayList;
import java.util.HashMap;

// V: vertex type, L: label type
public class BipartiteGraph<V,L> {

    private HashMap<V, ArrayList<Edge<V,L>>> adjacencyList;
    ...
    public BipartiteGraph(boolean isDirected) {
        directed = isDirected;
        adjacencyList = new HashMap<V, ArrayList<Edge<V,L>>>();
    ...
    public void add(V vertex, ArrayList<Edge<V,L>> connectedVertices) {
        ...
        for (Edge<V,L> vertexConnectedToAddedVertex : connectedVertices) {
            ...
            if (!directed) {
                // The weight from one vertex back to another in an undirected graph is equal
                int weight = vertexConnectedToAddedVertex.getWeight();
                correspondingConnectedList.add(new Edge<V,L>(vertex, weight));
            }
        }
    ...
    public boolean addEdge(V vertexOne, V vertexTwo, int weight) {
        ...
        if (!adjacencyList.containsKey(vertexOne)) {
            ArrayList<Edge<V,L>> tempList = new ArrayList<Edge<V,L>>();
            tempList.add(new Edge<V,L>(vertexTwo, weight));
        ...
        if (!adjacencyList.containsKey(vertexTwo)) {
            ArrayList<Edge<V,L>> tempList = new ArrayList<Edge<V,L>>();
            tempList.add(new Edge<V,L>(vertexOne, weight));
            add(vertexTwo, tempList);
            return true;
        }

        adjacencyList.get(vertexOne).add(new Edge<V,L>(vertexTwo, weight));
        adjacencyList.get(vertexTwo).add(new Edge<V,L>(vertexOne, weight));
        return true;
    }
